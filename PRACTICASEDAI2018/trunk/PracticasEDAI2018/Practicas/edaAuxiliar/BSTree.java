...
public class BSTree<T extends Comparable<T>> implements Collection<T>, Iterable<T>, Cloneable, BinarySearchTree<T> {
	...
	public String toStringBreadthFirstTraversal() {
		...
		q.push(current);  // initialize the queue with the root
		while(!q.isEmpty()) {
		  current = q.pop();
		  s += current.nodeValue + "  ";
		  if(current.left != null)
		     q.push(current.left); 	// insert the left in the queue
		  if(current.right != null)
		     q.push(current.right);	// insert the right in the queue
		}
		return s;
	}
	...
	/*
	 * Uses the method copyTree() in the class BinaryTree as a model to
	 ...
	 */
	private String preorderDisplay(BSTNode<T> t) {
		// return value
		String s = "";
	...
	public boolean contains(T item) {
		return this.find(item)!= null ? true : false;
	}
