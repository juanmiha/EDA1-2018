...
public class Network<Vertex extends Comparable<Vertex>> implements Graph<Vertex> {
	...
	public Network(){
  	...
  	public String getNeighborsOfVertex(Vertex v){
		...
		//"{<pepe, alberto>=63.0, <pepe, anabel>=47.0, <pepe, juan>=61.0, <pepe, maria>=85.0}");
		...
		reducedAdjacencyMap = new TreeMap<String, Double>();
		for (Vertex it : adjacencyMap.keySet()) {
			if(v.equals(it)){
				v = it;
				for(Entry<Vertex,Double> itV : adjacencyMap.get(v).entrySet()){
					reducedAdjacencyMap.put("<" + v.toString() + ", " + itV.getKey().toString() + ">",itV.getValue());
				}
			}
		}
