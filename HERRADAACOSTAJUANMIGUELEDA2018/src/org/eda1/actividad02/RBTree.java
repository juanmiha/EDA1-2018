...
public class RBTree<T extends Comparable<T>> implements Collection<T>, Iterable<T> {
	...
	private RBNode<T> findNode(T item)
	{
		// cycle t through the tree starting with root
		RBNode<T> t = root;

		// terminate on on empty subtree
		while(t != NIL && item.compareTo(t.nodeValue) != 0)
		{
			if (item.compareTo(t.nodeValue) < 0)
				t = t.left;
			else
				t = t.right;
		}
    ...
    */
	...
	private int pathHeight(RBNode<T> current, T item, int altura){
		if(current == null)return -1;
		if(current.nodeValue.compareTo(item)== 0) return altura;
		
		if(current.nodeValue.compareTo(item) > 0 && current.left != null)
			return pathHeight(current.left, item, altura+1);
		
		return pathHeight(current.right,item,altura+1);
	}
	...
	private static class RBNode<T extends Comparable<T>>
	{
		...
		//constructors
		public RBNode(T item, RBNode<T> left, RBNode<T> right, RBNode<T> parent, int color)
		{
			nodeValue = item;
			this.left = left;
			this.right = right;
