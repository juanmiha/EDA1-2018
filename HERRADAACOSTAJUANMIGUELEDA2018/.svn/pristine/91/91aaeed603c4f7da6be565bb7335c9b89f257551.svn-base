...
public class BSTree<T extends Comparable<T>> {
	...
	private void toArrayList(BSTNode<T> current, ArrayList<T> arr){
		if(current == null) return;
		toArrayList(current.left,arr);
		arr.add(current.nodeValue);
		toArrayList(current.right,arr);
	}
	...
	private String toStringLevel(BSTNode<T> t, int currentLevel, int level){
		...
		if(t != null){
			if(level == currentLevel)
				...
				salida += toStringLevel(t.left, currentLevel + 1, level) + toStringLevel(t.right,currentLevel +1 ,level);
		}
		return salida;
	}
	...
	private int height(BSTNode<T> current) {
		...
		vI = height(current.left);
		vD = height(current.right);
		return 1+ Math.max(vI, vD);
	}
	...
	private int numberOfLeaves(BSTNode<T> current) {
		...
		contador += numberOfLeaves(current.left);
		contador += numberOfLeaves(current.right);
		return contador;
	...
	private String code(BSTNode<T> current, T item, String codigo){
		if(current.nodeValue.compareTo(item)== 0) return codigo;

		if(current.nodeValue.compareTo(item)<0)
			return code(current.right,item,codigo+"1");

		return code(current.left,item,codigo+"0");
	}
